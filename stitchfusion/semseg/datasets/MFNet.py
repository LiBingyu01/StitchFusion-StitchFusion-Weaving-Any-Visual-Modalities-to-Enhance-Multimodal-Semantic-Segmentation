...
class MFNet(Dataset):
    ...
    def __init__(self, root: str = 'data/MFNet', split: str = 'train', transform = None, modals = ['rgb', 'ther'], case = None) -> None:
        super().__init__()
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        rgb = os.path.join(*[self.root, 'img', item_name+'.png']) # ./rgb/name
        x1 = os.path.join(*[self.root, 'ther', item_name+'.png']) # ./ther/name
        lbl_path = os.path.join(*[self.root, 'labels', item_name+'.png'])
        sample = {}
        sample['img'] = io.read_image(rgb)[:3, ...]
        if 'ther' in self.modals:
            sample['ther'] = self._open_img(x1)
        label = io.read_image(lbl_path)[0,...].unsqueeze(0)
        sample['mask'] = label
        
        if self.transform: # 是否变换
            sample = self.transform(sample)

        label = sample['mask'] # 标签
        del sample['mask']
    ...
    def _open_img(self, file): # 打开文件夹，之后读取文件，返回文件名
