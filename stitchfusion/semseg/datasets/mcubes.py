...
class MCubeS(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        # _aolp_sin = (np.load(x1) + 2.66) / (2.66 + 2.40)            # See below of this script for min-max values
        # _aolp_cos = (np.load(x1_1) + 0.57 ) / (0.57 + 1.64)
        _aolp_sin = np.load(x1)
        _aolp_cos = np.load(x1_1)
        _aolp = np.stack([_aolp_sin, _aolp_cos, _aolp_sin], axis=2) # H x W x 3
        dolp = (np.load(x2) + 0.55) / 2.0
        ...
        # del _img, _target, _aolp, _dolp, _nir, _nir_mask, _mask
        return sample, label
...
# Reading DoLP....
# DoLP: Global Min: -0.5281060934066772, Global Max: 1.4464844465255737, Negative Count: 11528302, Negative Percent: 1.839560036254085%
# Reading AoLP (Sin)....
# AoLP (Sin): Global Min: -2.6559877395629883, Global Max: 2.3968138694763184, Negative Count: 279532583, Negative Percent: 44.604744785283906%
# Reading AoLP (Sin)....
# AoLP (Cos): Global Min: -0.5681185722351074, Global Max: 1.6386538743972778, Negative Count: 3569347, Negative Percent: 0.5695572597528594%